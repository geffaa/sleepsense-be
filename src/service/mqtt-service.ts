...
export class MqttService {
  ...
  /**
   * Mengirim data sensor tunggal ke MQTT broker
   */
  sendSensorData(
    ...
    return new Promise((resolve, reject) => {
      ...
      const topic = `sleepsense/device/${deviceSerialNumber}/data`;
  ...
  /**
   * Mengirim batch data sensor ke MQTT broker
   */
  sendBatchSensorData(
    deviceSerialNumber: string,
    data: Array<{
    ...
    return new Promise((resolve, reject) => {
      ...
      const topic = `sleepsense/device/${deviceSerialNumber}/batch`;
      
      // Format data untuk batch
      const formattedData = data.map(item => ({
        ...
        radar_amplitude: item.radar_amplitude,
        has_apnea_event: item.has_apnea_event
      }));
      ...
      this.client.publish(topic, payload, { qos: 1 }, (err) => {
        if (err) {
          console.error('Error publishing batch sensor data:', err);
          reject(err);
        } else {
          resolve();
        }
...
//   .then(() => {
//     // Kirim data sensor
//     return mqttService.sendSensorData('SN12345', new Date(), {
//       ecg_mv: 1.25,
//       spo2: 98,
//       heart_rate: 75,
